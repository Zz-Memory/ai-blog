...
- **博主（Blogger）**：负责博客内容的创建、编辑、发布、分类标签管理以及评论审核、用户管理等后台运营工作。
...
博主可在编辑器中创建草稿、实时编辑、自动保存、上传 Markdown 文件、预览内容、生成标题、润色文本，并对文章进行发布、撤回、删除、分类和标签管理。
...
系统支持文章评论、回复、审核、删除，并在评论、系统消息、站内互动时生成通知提醒。
...
- **内容生产链路**：编辑器 + 草稿 + 发布 + 标签分类 + Markdown 导入
