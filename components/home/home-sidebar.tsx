  ...
  return (
    <aside className="flex w-[280px] flex-shrink-0 flex-col gap-6">
      <section className="rounded-2xl border border-white/8 bg-[#17181d] p-8 text-center shadow-[0_0_0_1px_rgba(255,255,255,0.02)]">
        ...
        <p className="mt-4 text-sm leading-7 text-zinc-400">记录数字生命轨迹，致力于探索人工智能与人类审美的交汇点，相信技术应当既无形，服务于心。</p>
        <a href="https://github.com/Zz-Memory" className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-zinc-400 transition hover:text-blue-200">
        ...
        <div className="space-y-3">
          {categories.map((category) => (
            <Link
              key={category.name}
              href={category.href}
              className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition ${category.active ? "border-blue-400/30 bg-blue-400/10 text-blue-100" : "border-white/8 bg-[#111215] text-zinc-400 hover:border-white/15 hover:bg-white/5 hover:text-zinc-200"}`}
            >
