...
export function HomeSidebar({ categories, hotTags }: HomeSidebarProps) {
  return (
    <aside className="flex w-[280px] flex-shrink-0 flex-col gap-6">
      <section className="rounded-2xl border border-sky-200 bg-white p-8 text-center shadow-[0_18px_45px_rgba(15,23,42,0.08)] backdrop-blur-sm">
        <div className="mx-auto h-24 w-24 overflow-hidden rounded-full border-4 border-sky-100 bg-sky-50">
        ...
        <p className="mt-2 text-sm font-medium text-sky-700">独立开发者 & AI 探索者</p>
        <p className="mt-4 text-sm leading-7 text-slate-700">记录数字生命轨迹，致力于探索人工智能与人类审美的交汇点，相信技术应当既无形，服务于心。</p>
        <a href="https://github.com/Zz-Memory" className="mt-6 inline-flex items-center gap-2 text-sm font-medium text-slate-600 transition hover:text-sky-700">
        ...
        <div className="space-y-3">
          {categories.map((category) => (
            <Link
              key={category.name}
              href={category.href}
              className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left transition ${category.active ? "border-sky-300 bg-sky-100 text-sky-800" : "border-sky-200 bg-white text-slate-600 hover:border-sky-300 hover:bg-sky-50 hover:text-sky-800"}`}
            >
