...
export function VisitorCenterAccountSettings() {
  ...
  return (
    <div className="space-y-8 rounded-[28px] border border-cyan-100/80 bg-white/90 p-6 shadow-[0_20px_60px_rgba(56,189,248,0.12)] sm:p-8">
      ...
      <section className="relative overflow-hidden rounded-2xl border border-cyan-100 bg-cyan-50/50 p-6 shadow-[0_20px_60px_rgba(56,189,248,0.08)] sm:p-8">{/* ... existing profile card unchanged ... */}
        <div className="absolute -right-24 -top-24 h-64 w-64 rounded-full bg-cyan-200/30 blur-[80px] pointer-events-none" />
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
          <div className="flex shrink-0 flex-col items-center gap-3"><div className="h-32 w-32 overflow-hidden rounded-full border border-cyan-100 bg-white"><img src={avatarUrl} alt={savedNickname} className="h-full w-full object-cover" /></div><span className="text-xs tracking-[0.2em] text-slate-500">用户头像</span></div>
          <div className="flex-1 space-y-4 pt-1">
            <div className="space-y-2"><label htmlFor="nickname" className="text-xs uppercase tracking-[0.18em] text-slate-500">昵称（可修改）</label>{nicknameEditing ? <input id="nickname" type="text" value={nicknameDraft} onChange={(event) => setNicknameDraft(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 text-sm text-slate-700 outline-none transition focus:border-cyan-300 focus:ring-1 focus:ring-cyan-200/40" /> : <button type="button" onClick={() => { setNicknameDraft(savedNickname); setNicknameEditing(true); }} className="flex w-full items-center justify-between rounded-xl border border-cyan-100 bg-white px-4 py-3 text-left text-sm text-slate-700 transition hover:border-cyan-300 hover:bg-cyan-50"><span>{savedNickname}</span><span className="material-symbols-outlined text-[18px] text-slate-400">edit</span></button>}</div>
            <div className="space-y-2"><label htmlFor="account" className="text-xs uppercase tracking-[0.18em] text-slate-500">账号（邮箱）</label><input id="account" type="text" disabled value={email} className="w-full cursor-not-allowed rounded-xl border border-transparent bg-white px-4 py-3 text-sm text-slate-500 opacity-90 outline-none" /></div>
          </div>
        </div>
        <div className="mt-6 flex items-center justify-between border-t border-cyan-100 pt-5"><p className="text-xs text-slate-500">{profileLoading ? "正在加载账号信息..." : "账户已同步到数据库"}</p><button type="button" disabled={profileSaving || profileLoading} onClick={handleSaveNickname} className="rounded-xl bg-gradient-to-r from-cyan-500 to-violet-500 px-5 py-3 text-sm font-medium text-white transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60">{profileSaving ? "保存中..." : "保存更改"}</button></div>
      </section>
      ...
      <section id="security" className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
          <div className="space-y-5 rounded-2xl border border-cyan-100 bg-cyan-50/50 p-6 shadow-[0_20px_60px_rgba(56,189,248,0.08)]">
            <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3 text-slate-900"><span className="material-symbols-outlined text-cyan-700">key</span><h3 className="text-base font-semibold">修改密码</h3></div><div className="flex rounded-lg border border-cyan-100 bg-white p-1 text-xs"><button type="button" onClick={() => setPasswordMode("modify")} className={`rounded-md px-3 py-1 transition ${passwordMode === "modify" ? "bg-cyan-50 text-cyan-700" : "text-slate-500"}`}>修改密码</button><button type="button" onClick={() => setPasswordMode("reset")} className={`rounded-md px-3 py-1 transition ${passwordMode === "reset" ? "bg-cyan-50 text-cyan-700" : "text-slate-500"}`}>重置密码</button></div></div>
            {passwordMode === "modify" ? <div className="space-y-3"> <div className="relative"><input value={oldPassword} onChange={(event) => setOldPassword(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 pr-12 text-sm text-slate-700 outline-none" placeholder="旧密码" type={showOldPassword ? "text" : "password"} /><button type="button" onClick={() => setShowOldPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-cyan-700" aria-label={showOldPassword ? "隐藏旧密码" : "显示旧密码"}><span className="material-symbols-outlined text-[18px]">{showOldPassword ? "visibility_off" : "visibility"}</span></button></div><div className="relative"><input value={newPassword} onChange={(event) => setNewPassword(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 pr-12 text-sm text-slate-700 outline-none" placeholder="新密码" type={showNewPassword ? "text" : "password"} /><button type="button" onClick={() => setShowNewPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-cyan-700" aria-label={showNewPassword ? "隐藏新密码" : "显示新密码"}><span className="material-symbols-outlined text-[18px]">{showNewPassword ? "visibility_off" : "visibility"}</span></button></div>{newPassword ? <p className={`text-xs leading-6 ${newPasswordError ? "text-rose-600" : "text-emerald-600"}`}>{newPasswordError || "密码需由字母与数字组合，且长度至少 8 位。"}</p> : <p className="text-xs leading-6 text-slate-500">密码需由字母与数字组合，且长度至少 8 位。</p>}<div className="relative"><input value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 pr-12 text-sm text-slate-700 outline-none" placeholder="确认新密码" type={showConfirmPassword ? "text" : "password"} /><button type="button" onClick={() => setShowConfirmPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-cyan-700" aria-label={showConfirmPassword ? "隐藏确认密码" : "显示确认密码"}><span className="material-symbols-outlined text-[18px]">{showConfirmPassword ? "visibility_off" : "visibility"}</span></button></div>{confirmPassword ? <p className={`text-xs leading-6 ${confirmPasswordError ? "text-rose-600" : "text-emerald-600"}`}>{confirmPasswordError || "密码需由字母与数字组合，且长度至少 8 位。"}</p> : <p className="text-xs leading-6 text-slate-500">密码需由字母与数字组合，且长度至少 8 位。</p>}<div className="flex justify-end"><button type="button" onClick={handleChangePassword} disabled={loadingAction === "password"} className="rounded-xl border border-cyan-100 bg-white px-4 py-2 text-sm text-cyan-800 transition hover:bg-cyan-50 disabled:opacity-60">{loadingAction === "password" ? "提交中..." : "更改密码"}</button></div></div> : <div className="space-y-3"><div className="flex gap-3"><input value={resetCode} onChange={(event) => setResetCode(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 text-sm text-slate-700 outline-none" placeholder="邮箱验证码" type="text" /><button type="button" onClick={() => sendVerificationCode("RESET_PASSWORD")} disabled={resetSendDisabled} className="shrink-0 rounded-xl border border-cyan-100 bg-white px-4 py-3 text-sm font-medium text-cyan-800 transition hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-60">{resetCooldown > 0 ? `${resetCooldown} 秒后可重发` : "发送验证码"}</button></div>{resetCode ? <p className={`text-xs leading-6 ${resetCodeError === "验证码正确" ? "text-emerald-600" : "text-rose-600"}`}>{resetCodeError}</p> : <p className="text-xs leading-6 text-slate-500">请输入邮箱收到的 6 位验证码。</p>}<div className="relative"><input value={newPassword} onChange={(event) => setNewPassword(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 pr-12 text-sm text-slate-700 outline-none" placeholder="新密码" type={showNewPassword ? "text" : "password"} /><button type="button" onClick={() => setShowNewPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-cyan-700" aria-label={showNewPassword ? "隐藏新密码" : "显示新密码"}><span className="material-symbols-outlined text-[18px]">{showNewPassword ? "visibility_off" : "visibility"}</span></button></div>{newPassword ? <p className={`text-xs leading-6 ${newPasswordError ? "text-rose-600" : "text-emerald-600"}`}>{newPasswordError || "密码需由字母与数字组合，且长度至少 8 位。"}</p> : <p className="text-xs leading-6 text-slate-500">密码需由字母与数字组合，且长度至少 8 位。</p>}<div className="relative"><input value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="w-full rounded-xl border border-cyan-100 bg-white px-4 py-3 pr-12 text-sm text-slate-700 outline-none" placeholder="确认新密码" type={showConfirmPassword ? "text" : "password"} /><button type="button" onClick={() => setShowConfirmPassword((value) => !value)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-cyan-700" aria-label={showConfirmPassword ? "隐藏确认密码" : "显示确认密码"}><span className="material-symbols-outlined text-[18px]">{showConfirmPassword ? "visibility_off" : "visibility"}</span></button></div>{confirmPassword ? <p className={`text-xs leading-6 ${confirmPasswordError ? "text-rose-600" : "text-emerald-600"}`}>{confirmPasswordError || "密码需由字母与数字组合，且长度至少 8 位。"}</p> : <p className="text-xs leading-6 text-slate-500">密码需由字母与数字组合，且长度至少 8 位。</p>}<div className="flex justify-end"><button type="button" onClick={handleResetPassword} disabled={loadingAction === "reset"} className="rounded-xl border border-cyan-100 bg-white px-4 py-2 text-sm text-cyan-800 transition hover:bg-cyan-50 disabled:opacity-60">{loadingAction === "reset" ? "提交中..." : "更改密码"}</button></div></div>}
          </div>
          <div className="relative overflow-hidden rounded-2xl border border-rose-200 bg-rose-50 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.06)]">
